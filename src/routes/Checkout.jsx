    ...
    if (sendOrders.data) {
      ...
      for (let item of cart) {
        let createProductOrderRelations = await supabase
          .from('product_order')
          .insert({
            orderId: sendOrders.data.id,
            productId: item.id
          }).select()

        console.log(createProductOrderRelations)
        if (createProductOrderRelations) {
          navigate('/error')
      ...
      {(loading)
        ...
        : <div >
          ...
          <div className="w-full bg-base-200 p-4 sticky bottom-0 min-h-[10rem] checkoutDescription flex flex-col justify-center items-center ">
            ...
            <button className="btn btn-primary" onClick={() => document.getElementById('my_modal_5').showModal()}>Continuar</button>
            <dialog id="my_modal_5" className="modal modal-bottom sm:modal-middle">
              <div className="modal-box">
                ...
                <form className="form flex flex-col justify-center items-center w-4/5 p-2" onSubmit={(e) => handleNotes(e)} method='dialog' >
                  <textarea className='textarea textarea-bordered textarea-primary mb-2 w-full focus:invalid:input-error' type='text' id='orderNotes' placeholder='Dirección: País, Provincia, Ciudad, Código Postal, Barrio, Calle' value={notes} onChange={(e) => setNotes(e.target.value)} />
